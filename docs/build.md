...
curl -fsSL https://nginx.org/download/nginx-1.31.3.tar.gz | tar -xz -C "$HOME/src"
( cd "$HOME/src/nginx-1.31.3" && ./configure --with-compat )
export NGINX_SOURCE_DIR="$HOME/src/nginx-1.31.3"
...
| `NGX_VERSION`          | Which nginx version to download if no source dir is given (needs `ngx/vendored`; defaults to `1.31.3` via [`.cargo/config.toml`](../.cargo/config.toml)). |
...
2. **`--with-compat` is the easy path.** Distro and official nginx.org packages
   ...
   ```bash
   export NGX_VERSION=1.31.3   # defaults to 1.31.3 via .cargo/config.toml
...
RUN curl -fsSL https://nginx.org/download/nginx-1.31.3.tar.gz | tar -xz -C /tmp \
 && (cd /tmp/nginx-1.31.3 && ./configure --with-compat) \
 && NGINX_SOURCE_DIR=/tmp/nginx-1.31.3 cargo build --release

FROM nginx:1.31.3          # MUST match the nginx built against above
...
| build can't locate nginx source                      | Set `NGINX_SOURCE_DIR`, or build `--features ngx/vendored` (downloads `NGX_VERSION`, default 1.31.3). |
