...
| Occasion | Title |
| --- | --- |
| Feature release | `v0.2.0 — per-path difficulty & challenge page theming (nginx 1.31.5)` |
| Bugfix release | `v0.1.1 — fix clearance cookie SameSite handling (nginx 1.31.5)` |
| Pure nginx rebuild | `v0.1.1 — rebuild for nginx 1.31.5` |
...
ngx_http_pow_gate_module-0.1.1-nginx1.31.5-glibc-amd64.so
```
...
1.31.5`") without a module version bump only if the module version at that tag
